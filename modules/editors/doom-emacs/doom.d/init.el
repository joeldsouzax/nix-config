...
(doom! :input
       ...
       :completion
       ;;company           ; [DEPRECATED for power users] moving to corfu
       (corfu +orderless +icons +init)  ; The modern standard. Faster, lighter, uses native Emacs childframes.
       (vertico +childframe +icons)     ; The best search engine for the minibuffer.
       ...
       (ligatures +fira +hasklig +iosevka +pragmata-pro)
       ...
       (vc-gutter +pretty +diff-hl) ; vcs diff in the fringe (diff-hl is faster than git-gutter)
       ...
       :emacs
       (dired +icons +dirvish)  ; making dired pretty [functional] (dirvish replaced ranger)
       ...
       direnv              ; ESSENTIAL for Nix integration
       (docker +lsp)
       ...
       (org +dragndrop +gnuplot +noter +hugo +present +pomodoro +pretty +roam2 +jupyter +ipython)
