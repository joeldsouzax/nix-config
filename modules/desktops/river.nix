...
with host; {
  ...
  config = mkIf (config.river.enable) {
    ...
    home-manager.users.${vars.user} = let
      monitor =
        "riverctl spawn ${pkgs.wlr-randr}/bin/wlr-randr --output ${secondMonitor} --mode 1920x1080@60 --pos 0,0 --transform 90 --output ${mainMonitor} --mode 3840x2160@60 --pos 1920,0";
    in {
      home.file = {
        ".config/river/init" = {
          executable = true;
